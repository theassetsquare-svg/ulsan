...
/* ---- sitemap.xml ---- */
{
  ...
  const missing = regions.filter((r) => !xml.includes(SITE + '/night-11/' + r.slug + '/'));
  if (missing.length) {
    const block = missing.map((r) =>
      '  <url>\n' +
      '    <loc>' + SITE + '/night-11/' + r.slug + '/</loc>\n' +
...
/* ---- llms.txt ---- */
{
  ...
  const missing = regions.filter((r) => !txt.includes('/night-11/' + r.slug + '/'));
  if (missing.length) {
    const oneLiner = (r) => {
      const age = r.age ? ' (연결 업소 출입 ' + r.age + ')' : '';
      return '- ' + SITE + '/night-11/' + r.slug + '/ — ' + r.kw + ' — ' + r.region + ' — ' +
