...
function extraChecks() {
  ...
  /* 전화번호 화이트리스트 */
  {
    ...
    Object.entries(pages).forEach(([k, p]) => {
      ...
      if (k === 'night/changwon-lululala-night') { allowed.add('연락처 삭제(미제휴)'); allowed.add('연락처 삭제(미제휴)'); allowed.add('연락처 삭제(미제휴)'); }
  ...
  /* JSON-LD telephone 은 광고주 3곳만 */
  {
    ...
    venues.forEach((v) => {
      ...
      const shouldHave = ['ulsan-champion-night', 'changwon-lululala-night', 'bulgwang-hobak'].includes(v.slug);
  ...
  /* 외부 링크 금지 (업소·허브) */
  {
    ...
    Object.entries(pages).forEach(([k, p]) => {
      ...
      const ext = [...p.src.matchAll(/href="(https?:\/\/[^"]*)"/g)].map((m) => m[1])
        .filter((u) => !u.startsWith('https://a.nolcool.com') && !u.startsWith('https://open.kakao.com/o/sBesta12') && !u.startsWith('http://schema.org') && !u.startsWith('https://schema.org'));
  ...
  /* G10 전화번호 위치 — 배포되는 모든 파일 스캔 (html·txt·xml·서빙 js) */
  {
    ...
    const OWNER = {
      ...
      '연락처 삭제(미제휴)': ['night/changwon-lululala-night/index.html'],
